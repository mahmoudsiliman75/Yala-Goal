<!-- START:: INCLUDING HEADER -->
<?php include('layouts/header.php') ?>
<!-- END:: INCLUDING HEADER -->

<!-- START:: CONTENT -->
<div class="news">
  <div class="container">
    <div class="row">

      <!-- START:: SIDE ADS -->
      <div class="side-add col-2 p-2 mt-2">
        <img class="img-fluid d-inline-block w-100 mb-2" src="imgs/ad.jpg">
        <img class="img-fluid d-inline-block w-100 mb-2" src="imgs/ad.jpg"> 
      </div>
      <!-- END:: SIDE ADS -->

      <!-- START:: NEWS CARDS -->
      <div class="news-container justify-content-center text-center col-8 p-3">
        <h2 class="custom-title"> أحدث الأخبار </h2>
        <div class="row">

          <!-- START:: NEWS CARD -->
          <div class="p-2 col-12 col-md-4">
            <div class="card">
              <div class="card-news-box">
                <img src="imgs/news/news1.jpg" class="card-img-top" alt="">
              </div>
              <div class="card-body">
                <p class="card-text">
                  <a href="#"> لوريم ايبسوم: دولار سيت أميت ,كونسيكتيتور أدايبا يسكينج أليايت.  </a> 
                </p>
              </div>
            </div>
          </div>
          <!-- END:: NEWS CARD -->

          <!-- START:: NEWS CARD -->
          <div class="p-2 col-12 col-md-4">
            <div class="card">
              <div class="card-news-box">
                <img src="imgs/news/news1.jpg" class="card-img-top" alt="">
              </div>
              <div class="card-body">
                <p class="card-text">
                  <a href="#"> لوريم ايبسوم: دولار سيت أميت ,كونسيكتيتور أدايبا يسكينج أليايت.  </a> 
                </p>
              </div>
            </div>
          </div>
          <!-- END:: NEWS CARD -->

          <!-- START:: NEWS CARD -->
          <div class="p-2 col-12 col-md-4">
            <div class="card">
              <div class="card-news-box">
                <img src="imgs/news/news1.jpg" class="card-img-top" alt="">
              </div>
              <div class="card-body">
                <p class="card-text">
                  <a href="#"> لوريم ايبسوم: دولار سيت أميت ,كونسيكتيتور أدايبا يسكينج أليايت.  </a> 
                </p>
              </div>
            </div>
          </div>
          <!-- END:: NEWS CARD -->

          <!-- START:: NEWS CARD -->
          <div class="p-2 col-12 col-md-4">
            <div class="card">
              <div class="card-news-box">
                <img src="imgs/news/news1.jpg" class="card-img-top" alt="">
              </div>
              <div class="card-body">
                <p class="card-text">
                  <a href="#"> لوريم ايبسوم: دولار سيت أميت ,كونسيكتيتور أدايبا يسكينج أليايت.  </a> 
                </p>
              </div>
            </div>
          </div>
          <!-- END:: NEWS CARD -->

          <!-- START:: NEWS CARD -->
          <div class="p-2 col-12 col-md-4">
            <div class="card">
              <div class="card-news-box">
                <img src="imgs/news/news1.jpg" class="card-img-top" alt="">
              </div>
              <div class="card-body">
                <p class="card-text">
                  <a href="#"> لوريم ايبسوم: دولار سيت أميت ,كونسيكتيتور أدايبا يسكينج أليايت.  </a> 
                </p>
              </div>
            </div>
          </div>
          <!-- END:: NEWS CARD -->

          <!-- START:: NEWS CARD -->
          <div class="p-2 col-12 col-md-4">
            <div class="card">
              <div class="card-news-box">
                <img src="imgs/news/news1.jpg" class="card-img-top" alt="">
              </div>
              <div class="card-body">
                <p class="card-text">
                  <a href="#"> لوريم ايبسوم: دولار سيت أميت ,كونسيكتيتور أدايبا يسكينج أليايت.  </a> 
                </p>
              </div>
            </div>
          </div>
          <!-- END:: NEWS CARD -->

          <!-- START:: NEWS CARD -->
          <div class="p-2 col-12 col-md-4">
            <div class="card">
              <div class="card-news-box">
                <img src="imgs/news/news1.jpg" class="card-img-top" alt="">
              </div>
              <div class="card-body">
                <p class="card-text">
                  <a href="#"> لوريم ايبسوم: دولار سيت أميت ,كونسيكتيتور أدايبا يسكينج أليايت.  </a> 
                </p>
              </div>
            </div>
          </div>
          <!-- END:: NEWS CARD -->

          <!-- START:: NEWS CARD -->
          <div class="p-2 col-12 col-md-4">
            <div class="card">
              <div class="card-news-box">
                <img src="imgs/news/news1.jpg" class="card-img-top" alt="">
              </div>
              <div class="card-body">
                <p class="card-text">
                  <a href="#"> لوريم ايبسوم: دولار سيت أميت ,كونسيكتيتور أدايبا يسكينج أليايت.  </a> 
                </p>
              </div>
            </div>
          </div>
          <!-- END:: NEWS CARD -->

          <!-- START:: NEWS CARD -->
          <div class="p-2 col-12 col-md-4">
            <div class="card">
              <div class="card-news-box">
                <img src="imgs/news/news1.jpg" class="card-img-top" alt="">
              </div>
              <div class="card-body">
                <p class="card-text">
                  <a href="#"> لوريم ايبسوم: دولار سيت أميت ,كونسيكتيتور أدايبا يسكينج أليايت.  </a> 
                </p>
              </div>
            </div>
          </div>
          <!-- END:: NEWS CARD -->

          <!-- START:: NEWS CARD -->
          <div class="p-2 col-12 col-md-4">
            <div class="card">
              <div class="card-news-box">
                <img src="imgs/news/news1.jpg" class="card-img-top" alt="">
              </div>
              <div class="card-body">
                <p class="card-text">
                  <a href="#"> لوريم ايبسوم: دولار سيت أميت ,كونسيكتيتور أدايبا يسكينج أليايت.  </a> 
                </p>
              </div>
            </div>
          </div>
          <!-- END:: NEWS CARD -->

          <!-- START:: NEWS CARD -->
          <div class="p-2 col-12 col-md-4">
            <div class="card">
              <div class="card-news-box">
                <img src="imgs/news/news1.jpg" class="card-img-top" alt="">
              </div>
              <div class="card-body">
                <p class="card-text">
                  <a href="#"> لوريم ايبسوم: دولار سيت أميت ,كونسيكتيتور أدايبا يسكينج أليايت.  </a> 
                </p>
              </div>
            </div>
          </div>
          <!-- END:: NEWS CARD -->

          <!-- START:: NEWS CARD -->
          <div class="p-2 col-12 col-md-4">
            <div class="card">
              <div class="card-news-box">
                <img src="imgs/news/news1.jpg" class="card-img-top" alt="">
              </div>
              <div class="card-body">
                <p class="card-text">
                  <a href="#"> لوريم ايبسوم: دولار سيت أميت ,كونسيكتيتور أدايبا يسكينج أليايت.  </a> 
                </p>
              </div>
            </div>
          </div>
          <!-- END:: NEWS CARD -->

        </div>
        <!-- START:: SHOW MORE BUTTON  -->
        <button class="btn btn-primary"> المزيد من الأخبار <i class="las la-angle-double-down"></i> </button>
        <!-- END:: SHOW MORE BUTTON  -->
      </div>
      <!-- START:: NEWS CARDS -->

      <!-- START:: SIDE ADS -->
      <div class="side-add col-2 p-2 mt-2" style="direction: ltr;">
        <img class="img-fluid d-inline-block w-100 mb-2" src="imgs/ad.jpg">
        <img class="img-fluid d-inline-block w-100 mb-2" src="imgs/ad.jpg"> 
      </div>
      <!-- END:: SIDE ADS -->
    </div>
  </div>
</div>
<!-- END:: CONTENT -->

<!-- START:: INCLUDING FOOTER -->
<?php include("layouts/footer.php") ?>
<!-- END:: INCLUDING FOOTER -->